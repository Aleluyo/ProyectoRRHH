    ...
    <div class="mb-5">
      <nav class="flex items-center gap-3 text-sm">
        ...
        <a href="<?= url('index.php?controller=postulacion&action=index') ?>"
          class="text-muted-ink hover:text-vc-ink transition">Postulaciones</a>
        <svg class="w-4 h-4 text-vc-peach" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
        </svg>
        <span class="font-medium text-vc-pink">Editar postulación
          #<?= (int) ($postulacion['id_postulacion'] ?? 0) ?></span>
      </nav>
    </div>
      ...
      <form id="formPostulacion" method="post"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <input id="fecha_postulacion" name="fecha_postulacion" type="date" required
        ...
        <div>
          ...
          <textarea id="comentarios" name="comentarios" rows="4"
            class="w-full rounded-lg border border-black/10 bg-white/80 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-vc-teal/60"><?= e((string) $old['comentarios']) ?></textarea>
