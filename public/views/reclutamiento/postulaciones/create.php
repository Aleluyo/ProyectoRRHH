...
<head>
  <meta charset="UTF-8" />
  <title>Nueva postulación · Reclutamiento</title>
...
<body class="min-h-screen bg-white text-vc-ink font-sans relative">
  ...
  <main class="mx-auto max-w-4xl px-4 sm:px-6 py-8 relative">
    ...
    <section class="rounded-xl border border-black/10 bg-white/90 p-5 shadow-soft">
      <form method="post" action="<?= url('index.php?controller=postulacion&action=store') ?>" class="space-y-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" name="id_vacante" id="id_vacante"
              value="<?= htmlspecialchars((string)($old['id_vacante'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"
              class="w-full rounded-lg border border-black/10 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-vc-teal/60"
              required>
            <?php if (!empty($errors['id_vacante'])): ?>
              <p class="mt-1 text-xs text-red-600"><?= htmlspecialchars($errors['id_vacante'], ENT_QUOTES, 'UTF-8') ?></p>
            <?php endif; ?>
          </div>
          ...
          <div>
            ...
            <input type="number" name="id_candidato" id="id_candidato"
              value="<?= htmlspecialchars((string)($old['id_candidato'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"
              class="w-full rounded-lg border border-black/10 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-vc-teal/60"
              required>
            <?php if (!empty($errors['id_candidato'])): ?>
              <p class="mt-1 text-xs text-red-600"><?= htmlspecialchars($errors['id_candidato'], ENT_QUOTES, 'UTF-8') ?></p>
            <?php endif; ?>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <?php $etapaOld = strtoupper((string)($old['etapa'] ?? 'SCREENING')); ?>
            <select name="etapa" id="etapa"
              class="w-full rounded-lg border border-black/10 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-vc-teal/60">
              <option value="SCREENING"  <?= $etapaOld === 'SCREENING'  ? 'selected' : '' ?>>Screening</option>
              <option value="ENTREVISTA" <?= $etapaOld === 'ENTREVISTA' ? 'selected' : '' ?>>Entrevista</option>
              <option value="PRUEBA"     <?= $etapaOld === 'PRUEBA'     ? 'selected' : '' ?>>Prueba</option>
              <option value="OFERTA"     <?= $etapaOld === 'OFERTA'     ? 'selected' : '' ?>>Oferta</option>
              <option value="CONTRATADO" <?= $etapaOld === 'CONTRATADO' ? 'selected' : '' ?>>Contratado</option>
              <option value="DESCARTADO" <?= $etapaOld === 'DESCARTADO' ? 'selected' : '' ?>>Descartado</option>
            </select>
          </div>
          ...
          <div>
            ...
            <input type="date" name="fecha_postulacion" id="fecha_postulacion"
              value="<?= htmlspecialchars((string)($old['fecha_postulacion'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"
              class="w-full rounded-lg border border-black/10 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-vc-teal/60">
          </div>
        </div>
        ...
        <div>
          ...
          <textarea name="comentarios" id="comentarios" rows="4"
            class="w-full rounded-lg border border-black/10 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-vc-teal/60"
          ><?= htmlspecialchars((string)($old['comentarios'] ?? ''), ENT_QUOTES, 'UTF-8') ?></textarea>
        </div>
