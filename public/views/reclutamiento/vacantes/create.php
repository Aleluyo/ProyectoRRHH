...
// $errors y $old vienen desde VacanteController::create(), pero por si acaso:
...
<head>
  <meta charset="UTF-8" />
  <title>Nueva vacante · Reclutamiento</title>
...
<body class="min-h-screen bg-white text-vc-ink font-sans relative">
  ...
  <main class="mx-auto max-w-4xl px-4 sm:px-6 py-8 relative">
    ...
    <!-- Título -->
    <section class="mb-6">
      <h1 class="vice-title text-[32px] leading-tight text-vc-ink">Nueva vacante</h1>
      <p class="mt-1 text-sm sm:text-base text-muted-ink">
        Registra una nueva requisición de vacante indicando área, puesto, ubicación y detalles.
      </p>
    </section>

    <!-- Formulario -->
    <section class="rounded-xl border border-black/10 bg-white/90 p-5 shadow-soft">
      <form method="post" action="<?= url('index.php?controller=vacante&action=store') ?>" class="space-y-4">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input
              ...
              name="id_area"
              id="id_area"
              value="<?= htmlspecialchars((string)($old['id_area'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"
          ...
          <div>
            ...
            <input
              ...
              name="id_puesto"
              id="id_puesto"
              value="<?= htmlspecialchars((string)($old['id_puesto'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"
              class="w-full rounded-lg border border-black/10 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-vc-teal/60"
              required
            >
          ...
          <div>
            ...
            <input
              ...
              name="id_ubicacion"
              id="id_ubicacion"
              value="<?= htmlspecialchars((string)($old['id_ubicacion'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"
              class="w-full rounded-lg border border-black/10 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-vc-teal/60"
              required
            >
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              name="solicitada_por"
              id="solicitada_por"
              value="<?= htmlspecialchars((string)($old['solicitada_por'] ?? ($_SESSION['user_id'] ?? '')), ENT_QUOTES, 'UTF-8') ?>"
              class="w-full rounded-lg border border-black/10 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-vc-teal/60"
              required
            >
          ...
          <div>
            ...
            <?php
              $estatusOld = strtoupper((string)($old['estatus'] ?? 'EN_APROBACION'));
            ?>
            <select
              name="estatus"
              id="estatus"
              ...
              required
            >
              <option value="EN_APROBACION" <?= $estatusOld === 'EN_APROBACION' ? 'selected' : '' ?>>En aprobación</option>
              <option value="ABIERTA"       <?= $estatusOld === 'ABIERTA'       ? 'selected' : '' ?>>Abierta</option>
              <option value="EN_PROCESO"    <?= $estatusOld === 'EN_PROCESO'    ? 'selected' : '' ?>>En proceso</option>
              <option value="CERRADA"       <?= $estatusOld === 'CERRADA'       ? 'selected' : '' ?>>Cerrada</option>
            </select>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              value="<?= htmlspecialchars((string)($old['fecha_publicacion'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"
              class="w-full rounded-lg border border-black/10 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-vc-teal/60"
            >
        ...
        <div>
          ...
          <textarea
            ...
            rows="4"
            class="w-full rounded-lg border border-black/10 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-vc-teal/60"
          ><?= htmlspecialchars((string)($old['requisitos'] ?? ''), ENT_QUOTES, 'UTF-8') ?></textarea>
