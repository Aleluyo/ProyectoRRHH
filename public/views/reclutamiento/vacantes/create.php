        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <?php if (!empty($errors['id_ubicacion'])): ?>
              <p class="mt-1 text-xs text-red-600"><?= htmlspecialchars($errors['id_ubicacion'], ENT_QUOTES, 'UTF-8') ?>
              </p>
            <?php endif; ?>
          </div>
          ...
          <div>
            ...
            <select id="solicitada_por" name="solicitada_por"
              class="mt-1 block w-full rounded-lg border border-black/10 bg-white/80 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-vc-teal/60">
              <option value="">Selecciona un usuario...</option>
              <?php foreach (($usuarios ?? []) as $u): ?>
                <?php
                $idU = (string) $u['id_usuario'];
                $nom = htmlspecialchars($u['username'] ?? ('Usuario #' . $idU), ENT_QUOTES, 'UTF-8');
                ?>
                <option value="<?= $idU ?>" <?= $oldSolicita === $idU ? 'selected' : '' ?>>
                  <?= $nom ?>
                </option>
              <?php endforeach; ?>
            </select>
            <?php if (!empty($errors['solicitada_por'])): ?>
              <p class="mt-1 text-xs text-red-600"><?= htmlspecialchars($errors['solicitada_por'], ENT_QUOTES, 'UTF-8') ?>
              </p>
            <?php endif; ?>
          </div>
