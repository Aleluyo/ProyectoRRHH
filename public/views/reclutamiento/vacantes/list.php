...
// Asegura que $vacantes siempre sea un array
...
<head>
  <meta charset="UTF-8" />
  <title>Vacantes · Reclutamiento y Selección</title>
...
<body class="min-h-screen bg-white text-vc-ink font-sans relative">
  ...
  <main class="mx-auto max-w-7xl px-4 sm:px-6 py-8 relative">
    ...
    <div class="mb-5">
      <nav class="flex items-center gap-3 text-sm">
        ...
        <span class="font-medium text-vc-pink">Vacantes</span>
      </nav>
    </div>
    ...
    <section class="mt-6">
      <div class="overflow-x-auto rounded-xl border border-black/10 bg-white/90 shadow-soft">
        <table class="min-w-full text-sm" id="tablaVacantes">
  ...
  <script>
    ...
    function formatEstatusBadge(estatus) {
      const e = (estatus || '').toString().toUpperCase();
      let bg = 'bg-slate-100 text-slate-700';

      if (e === 'EN_APROBACION') bg = 'bg-amber-100 text-amber-800';
      else if (e === 'APROBADA' || e === 'ABIERTA') bg = 'bg-emerald-100 text-emerald-800';
      else if (e === 'EN_PROCESO') bg = 'bg-sky-100 text-sky-800';
      else if (e === 'CERRADA') bg = 'bg-slate-200 text-slate-800';
            ...
            <td class="px-3 py-2 whitespace-nowrap">
              <div class="flex gap-2 justify-center">
                ...
                <button
                  ...
                  data-id="${idVac}"
                  data-label="Vacante #${idVac}"
                  data-href="${deleteBaseUrl}&id=${idVac}"
                >
                  Eliminar
                </button>
              </div>
    ...
    searchInput.addEventListener('input', () => {
      ...
      if (term === '') {
        filteredData = [...vacantesData];
      } else {
        filteredData = vacantesData.filter(vac =>
    ...
    document.addEventListener('click', event => {
      ...
      Swal.fire({
        ...
        html: label
          ...
          : 'Se eliminará la vacante seleccionada.',
        icon: 'warning',
