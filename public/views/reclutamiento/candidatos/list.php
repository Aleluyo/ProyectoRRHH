...
// Asegura que $candidatos siempre sea un array
...
<head>
  <meta charset="UTF-8" />
  <title>Candidatos · Reclutamiento y Selección</title>
...
<body class="min-h-screen bg-white text-vc-ink font-sans relative">
  ...
  <main class="mx-auto max-w-7xl px-4 sm:px-6 py-8 relative">
    ...
    <div class="mb-5">
      <nav class="flex items-center gap-3 text-sm">
        ...
        <span class="font-medium text-vc-pink">Candidatos</span>
      </nav>
    </div>
    ...
    <section class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <h1 class="vice-title text-[36px] leading-tight text-vc-ink">Candidatos</h1>
        <p class="mt-1 text-sm sm:text-base text-muted-ink">
          Banco de candidatos · datos de contacto y fuente de reclutamiento.
        </p>
      </div>
      ...
      <div class="flex flex-col sm:flex-row gap-3 sm:items-center">
        ...
        <div class="relative">
          <input
            ...
            placeholder="Buscar por nombre, correo, fuente…"
          />
        ...
        <!-- Botón de agregar -->
        <a
          href="<?= url('index.php?controller=candidato&action=create') ?>"
    ...
    <section class="mt-6">
      <div class="overflow-x-auto rounded-xl border border-black/10 bg-white/90 shadow-soft">
        <table class="min-w-full text-sm" id="tablaCandidatos">
          <thead class="bg-slate-100/80 text-xs uppercase tracking-wide text-muted-ink">
            <tr>
              <th class="px-3 py-2 text-left cursor-pointer select-none" data-sort="id_candidato">ID</th>
              <th class="px-3 py-2 text-left cursor-pointer select-none" data-sort="nombre">Nombre</th>
              <th class="px-3 py-2 text-left cursor-pointer select-none" data-sort="correo">Correo</th>
              <th class="px-3 py-2 text-left cursor-pointer select-none" data-sort="telefono">Teléfono</th>
              <th class="px-3 py-2 text-left cursor-pointer select-none" data-sort="fuente">Fuente</th>
              <th class="px-3 py-2 text-center">Acciones</th>
            </tr>
          </thead>
          <tbody id="tbodyCandidatos" class="align-top bg-white">
  ...
  <script>
    ...
    const candidatosData = <?= json_encode($candidatos, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?> || [];

    const rowsPerPage = 10;
    let currentPage = 1;
    let sortField = 'id_candidato';
    let sortDir = 'asc';
    let filteredData = [...candidatosData];
    ...
    const editBaseUrl   = "<?= url('index.php?controller=candidato&action=edit') ?>";
    const deleteBaseUrl = "<?= url('index.php?controller=candidato&action=delete') ?>";
            ...
            <td class="px-3 py-2 whitespace-nowrap">
              <div class="flex gap-2 justify-center">
                ...
                <button
                  ...
                  data-id="${id}"
                  data-label="${nombre || 'Candidato #' + id}"
                  data-href="${deleteBaseUrl}&id=${id}"
                >
                  Eliminar
                </button>
              </div>
    ...
    searchInput.addEventListener('input', () => {
      ...
      if (term === '') {
        filteredData = [...candidatosData];
      } else {
        filteredData = candidatosData.filter(c =>
    ...
    document.addEventListener('click', event => {
      ...
      Swal.fire({
        ...
        html: label
          ...
          : 'Se eliminará el candidato seleccionado.',
        icon: 'warning',
