 ...
 <!-- Breadcrumb -->
  <div class="mb-5">
    ...
    <section class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <h1 class="vice-title text-[36px] leading-tight text-vc-ink">Empresas</h1>
        <p class="mt-1 text-sm sm:text-base text-muted-ink">
          Catálogo de empresas · Razón social, RFC, contacto y más.
        </p>
      </div>
    ...
    <section class="mt-6">
      <div class="overflow-x-auto rounded-xl border border-black/10 bg-white/90 shadow-soft">
        <table class="min-w-full text-sm" id="tablaEmpresas">
          <thead class="bg-slate-100/80 text-xs uppercase tracking-wide text-muted-ink">
            <tr>
              <th class="px-3 py-2 text-left cursor-pointer select-none" data-sort="id_empresa">ID</th>
              <th class="px-3 py-2 text-left cursor-pointer select-none" data-sort="nombre">Nombre</th>
              <th class="px-3 py-2 text-left cursor-pointer select-none" data-sort="rfc">RFC</th>
              <th class="px-3 py-2 text-left cursor-pointer select-none" data-sort="correo_contacto">Correo contacto</th>
          ...
          <tbody id="tbodyEmpresas" class="align-top bg-white">
   ...
   const toggleBaseUrl = "<?= url('index.php?controller=empresa&action=toggle') ?>";
    ...
    function renderTable() {
      ...
      tbody.innerHTML = pageItems.map(emp => {
        ...
        const telefono  = escapeHtml(emp.telefono ?? '');
        const direccion = escapeHtml(emp.direccion ?? '');

        return `
          <tr class="border-t border-slate-200 hover:bg-slate-50">
            <td class="px-3 py-2 whitespace-nowrap text-xs text-muted-ink">${emp.id_empresa}</td>
            <td class="px-3 py-2 whitespace-nowrap">${nombre}</td>
            <td class="px-3 py-2 whitespace-nowrap text-xs">${rfc}</td>
    ...
    searchInput.addEventListener('input', () => {
      ...
      if (term === '') {
        filteredData = [...empresasData];
      } else {
        filteredData = empresasData.filter(emp =>
