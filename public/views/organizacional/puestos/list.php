...
// Obtención de datos
// -----------------------------------------------------------------------------
// Asegura que $puestos siempre sea un array (vacío si no viene definido)
...
<head>
  <meta charset="UTF-8" />
  <title>Puestos · Administración organizacional</title>
...
<body class="min-h-screen bg-white text-vc-ink font-sans relative">
  ...
  <main class="mx-auto max-w-7xl px-4 sm:px-6 py-8 relative">
    ...
    <div class="mb-5">
      <nav class="flex items-center gap-3 text-sm">
        ...
        <span class="font-medium text-vc-pink">Puestos</span>
      </nav>
    </div>
    ...
    <section class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <h1 class="vice-title text-[36px] leading-tight text-vc-ink">Puestos</h1>
        <p class="mt-1 text-sm sm:text-base text-muted-ink">
          Catálogo de puestos · Áreas, niveles jerárquicos y salarios.
        </p>
      </div>
      ...
      <div class="flex flex-col sm:flex-row gap-3 sm:items-center">
        ...
        <!-- Botón de agregar -->
        <a
          href="<?= url('index.php?controller=puesto&action=create') ?>"
    ...
    <section class="mt-6">
      <div class="overflow-x-auto rounded-xl border border-black/10 bg-white/90 shadow-soft">
        <table class="min-w-full text-sm" id="tablaPuestos">
          <thead class="bg-slate-100/80 text-xs uppercase tracking-wide text-muted-ink">
            <tr>
              <th class="px-3 py-2 text-left cursor-pointer select-none" data-sort="id_puesto">ID</th>
              <th class="px-3 py-2 text-left cursor-pointer select-none" data-sort="nombre_empresa">Empresa / Área</th>
              <th class="px-3 py-2 text-left cursor-pointer select-none" data-sort="nombre_puesto">Puesto</th>
          ...
          <tbody id="tbodyPuestos" class="align-top bg-white">
  ...
  <script>
    ...
    const puestosData = <?= json_encode($puestos, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?> || [];

    const rowsPerPage = 10;
    let currentPage = 1;
    let sortField = 'id_puesto';
    let sortDir = 'asc';
    let filteredData = [...puestosData];
    ...
    function renderTable() {
      ...
      tbody.innerHTML = pageItems.map(p => {
        const idPuesto       = p.id_puesto;
        const idArea         = p.id_area;
        const areaNombre     = escapeHtml(p.nombre_area ?? '');
        const empresaNombre  = escapeHtml(p.nombre_empresa ?? '');

        const nombre      = escapeHtml(p.nombre_puesto ?? '');
        ...
        if (p.salario_base !== null && p.salario_base !== undefined && p.salario_base !== '') {
          ...
          salarioBase = Number.isNaN(num)
            ? escapeHtml(p.salario_base)
            : num.toFixed(2);
        }

        return `
          <tr class="border-t border-slate-200 hover:bg-slate-50">
            <td class="px-3 py-2 whitespace-nowrap text-xs text-muted-ink">${idPuesto}</td>
            <td class="px-3 py-2 whitespace-nowrap text-xs text-muted-ink" title="ID área: ${idArea}">
              <div class="font-medium">${empresaNombre || '—'}</div>
              <div class="text-[11px] text-slate-500">${areaNombre || 'Sin área'}</div>
            </td>
            <td class="px-3 py-2 whitespace-nowrap">${nombre}</td>
            <td class="px-3 py-2 whitespace-nowrap text-xs">
              <span class="inline-flex items-center rounded-full border border-black/10 px-2 py-0.5 text-[11px] uppercase tracking-wide">
                ${nivel}
              </span>
            </td>
            <td class="px-3 py-2 whitespace-nowrap text-xs">
    ...
    searchInput.addEventListener('input', () => {
      ...
      if (term === '') {
        filteredData = [...puestosData];
      } else {
        filteredData = puestosData.filter(p =>
